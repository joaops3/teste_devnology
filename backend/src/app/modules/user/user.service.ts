import { Injectable, HttpException, BadRequestException, NotFoundException, UseGuards, Options } from '@nestjs/common';
...
export class UserService {
  ...
  async findOne(data: { _id?: string, email?: string }): Promise<User> {
    const user = await this.userModel
      .findOne(data).populate({ path: 'link', select: ['_id', 'title', 'href', 'createdAt'], options: { sort: { 'createdAt': -1 } } }).exec()
    if (!user) {
